...
def Input_file_name():
    while True:
        file_name = str(pyautogui.prompt('번역하실 mp3 파일을 바탕화면에 놓은 뒤 파일 이름을 기입해주세요.'
                                         '\n(확장자는 생략)'))
        if os.path.isfile("C:/Users/{}/Desktop/{}.mp3".format(id, file_name)) is True:
            break
        elif file_name == 'None':
            print(f"[{datetime.today()}] exit")
            sys.exit()
        else:
            r = pyautogui.confirm('바탕화면에서 {}.mp3 파일을 찾을 수 없습니다.'.format(file_name))
            if r == 'Cancel':
                print(f"[{datetime.today()}] exit")
                sys.exit()
            else:
...
def Convert_mp3_to_wav():
    sound = pydub.AudioSegment.from_mp3("C:/Users/{}/Desktop/{}.mp3".format(id, file_name))
